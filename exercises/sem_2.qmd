...
3. En utilisant la fonction `lapply`, calculez le prix minimal, médian, moyen, et maximal des biens par nombre de salles de bain. 
4. Utiliser `sapply`, en lieu et place de `lapply`. Comparez les résultats en utilisant `simplify = TRUE` puis `simplify = FALSE` comme argument de la fonction `sapply`.  Expliquez la différence.
...
11. Répondez à l'intégralité des questions à nouveaux en utilisant les fonctions `map_*` et `pmap_*` adaptées de la librairie `purrr`. Quels sont les différences entre les fonctions de `purrr` et les fonctions de base ? Quels sont les avantages de `purrr` ?
