...
3. Documentez les deux fonctions en utilisant des balises roxygen2. Veillez à spécifier que seule la fonction `heron` soit exportée. On précisera que la documentation de la fonction `demi_perimetre` doit être cachée en utilisant la balise `@noRd`. Ajoutez deux exemples à la fonction héron.
4. Modifiez la fonction `heron` pour que la fonction envoie une erreur si un des arguments n'est pas un numérique, ou si une des valeurs est négative. De même, vous renverrez un warning si une des valeurs est égale à 0.
5. Testez la fonction `demi_perimetre` avec trois tests. La fonction `heron` devra être testée sur chacune de ses erreurs, ainsi que pour plusieurs exemples de triangles.
6. Modifiez le fichier DESCRIPTION. Assurez vous que les tests passent bien et que la documentation est à jour, et installez votre package. Vérifiez que l'aide est accessible et que la fonction marche comme prévu.
7. Créez un nouveau projet github, que vous appelerez `heron`. 
8. Dans le terminal, initialisez un projet git en appelant la commande `git init`. Configurez le `remote` de manière à le faire pointer vers le projet github récemment créé. L'instruction devrait ressembler à `git remote add git@github.com:<votre username github>/heron.git`.
9. Ajoutez les fichiers du dossier au prochain commit, faites un commit, puis un push. Attention, vérifiez que vous n'ajoutez pas de fichiers confidentiels !
10. Fermez le projet RStudio. Dans un terminal R, installez le packages `remotes`. Que se passe t'il quand vous exécutez la commande `remotes::install_github("<votre username>/heron")` (remplacez `<votre username>` par votre username github) ?
...
5. Installez le package `heron` depuis votre dépôt github en utilisant le package `remotes`. Utilisez `renv::snapshot()` et vérifier que le package `heron` est bien mentionné dans votre fichier `renv.lock` à la racine de votre projet de blog.
6. Créez un fichier `_targets.R` à la racine du dossier `posts/post-with-code/` en utilisant la fonction `tar_script` et l'argument `script`.
7. Configurez `targets` pour référencer le bon fichier de pipeline en utilisant la fonction `tar_config_set` et en affectant à l'argument `script`  et `store`le chemin du fichier `_targets` et du dossier `_targets/`, ici normalement `posts/post-with-code/_targets.R`.
...
8. Soit A, B et C trois points de coordonnées $(0, 0), (0, 1), (0.5, \sqrt{3}/2)$. Supprimez les étapes non pertinentes et les imports non nécessaires et ajoutez trois étapes dans le fichier `_targets` qui correspondent à 4 itérations successives de `divide_list_triangle` à partir des points d'origine.
9. Implémentez une fonction `plot_triangles` qui trace une liste de triangle à la manière de @fig-triangle dans le fichier `fonctions.R`.
10. Ajoutez à la suite de chaque itération une étape de plot pour tracer les graphiques à l'issue de chaque itération.
11. Ajoutez à la suite de chaque itération une étape de calcul des aires de tous les triangles en utilisant la fonction `heron` du package `heron`. Cela pourra nécéssiter l'écriture d'une nouvelle fonction qui peut calculer l'aire de tous les triangles d'une liste et renverra la somme.
12. Modifiez votre post de blog `posts/post-with-code/index.qmd` de manière à rajouter une section par itération, afficher un graphique et la mesure de l'aire. On utilisera les fonction `tar_read` et `tar_load` pour charger les résultats directement depuis les objets calculés par la pipeline targets.
13. Ajouter une étape de render du post de blog dans la pipeline targets en utilisant la fonction `tar_quarto` de la librairie `tarchetypes`.
14. Ajoutez un fichier `.gitignore` à la racine du dossier `posts/post-with-code/`. Ajoutez la ligne suivante pour ne pas traquer les fichiers intermédiaires targets :

```bash
_targets/*
```
15. Exécutez une dernière fois la commande `tar_make`. Dans le terminal ou dans la console R, faites une étape de preview de votre site complet. Par exemple, on pourra utiliser la commande `quarto preview` dans le terminal.

16. Vérifiez que vous ajouter bien tous les documents et les dossiers à votre commit. Faire un commit, en particulier assurez vous que votre dossier `_site` est bien commit.
17. Faites un git push et vérifiez que votre projet est à jour sur github.
18. Configurez github pages. Assurez-vous que votre projet est public. Configurez votre `github pages` à la manière de :
